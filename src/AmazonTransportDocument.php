    ...
    public function save($path)
    {
        if (isset($this->doc)) {
            try {
                ...
                $this->log('Successfully saved transport document file ShipmentId #'.$this->options['ShipmentId']." at $path");
                return true;
            } catch (Exception $ex) {
                $this->log('Unable to save transport document file ShipmentId #'.$this->options['ShipmentId'].' Error: '.$ex->getMessage(), 'Urgent');
            }
        }
        return false;
    }
